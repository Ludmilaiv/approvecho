...
export const RecoveryBook = ({api, book, booking, setBooking}: props) => {
  ...
  const removeBook = async() => {
    ...
    setLoading(true);
    const newBook = await Methods.recoveryBook(api, book.id).then();
    if (!newBook) {
      setLoading(false);
  ...
  const form = <form action='#' onSubmit={e => {
    ...
    <p>{'Вы уверены, что восстановить заказ?'}</p>
  ...
  const doneContent = <div>
    <p>{'Заказ успешно восстановлен'}</p>
    <div className='modal__buttons'>
  ...
  return (
    <div>
      <h2 className='modal__header'>Восстановление заказа</h2>
